// Reçoit le webhook GDPR "customers/redact"
// Action: purger/supprimer les données personnelles du client dans ta base
...
serve(async (req) => {
  ...
  try {
    ...
    // 2) Purge / anonymisation (exemples, adapte selon ton schéma)
    const customerId = payload.customer && (payload.customer.id || payload.customer.shopify_customer_id);
    if (customerId) {
      // Exemple d'anonymisation douce:
      await supabase.from("customers")
        .update({
          email: null,
          first_name: null,
          last_name: null,
          // autres PII à nullifier
        })
        .eq("shopify_customer_id", customerId);

      // Si tu dois supprimer complètement :
      // await supabase.from("customers").delete().eq("shopify_customer_id", customerId);
    }
